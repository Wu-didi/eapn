...
(() => {
  ...
  section.innerHTML = `<div class="case-heading"><div><h3>R1 · 双臂叠衣</h3><small>AGILEX-PIPER · 原始完整视频 · 30 帧/秒</small></div><span class="real-badge">真机实验</span></div>
    <div class="clips">${clips.map(c => `<div class="clip"><div class="clip-head"><b class="real-${c.method}">${c.method === 'ours' ? '我们的方法' : '对比基线'}</b><span>记录 ${c.collection_id}</span></div><div class="video-wrap"><video muted playsinline preload="metadata" poster="${escape(c.poster)}" src="${escape(c.src)}" aria-label="真机 ${escape(c.label)}"></video></div><div class="video-foot"><span class="real-clip-time">0.00 秒</span><span>视频时长 ${c.duration_s.toFixed(2)} 秒</span></div></div>`).join('')}</div>
    <div class="controls"><div class="buttons"><button class="play" type="button">同步播放</button><button class="reset" type="button">从头</button><button class="prev" type="button">−1 帧</button><button class="next" type="button">+1 帧</button><select class="real-rate" aria-label="真机视频播放速度"><option value="1">1× · 原速</option><option value="0.5">0.5× · 慢放</option><option value="0.25">0.25× · 细看</option></select><span class="step-readout" aria-live="off"></span></div><div class="timeline"><span>0 秒</span><input class="real-timeline" type="range" min="0" max="${endTime}" value="0" step="${step}" aria-label="真机共同视频时间（秒）"><span>${endTime.toFixed(1)} 秒</span></div><div class="error" role="status"></div></div>
    ...
    <details><summary>关节数据、URDF 与对比说明</summary><p>每份 CSV 含时间列，以及左右臂各 7 个状态和动作通道，已核对与对应 JSON 标注逐行一致。TCP 使用左右臂各 6 个实测旋转关节状态，经原推理流程中的 URDF 正运动学计算到夹爪对称中心；不使用动作指令，也不对轨迹做平滑。全部 ${clips[0].frames + clips[1].frames} 帧的双臂位置已与原推理流程的计算实现交叉核对。</p><p>关节时间戳存在跳步，与固定 30 帧/秒视频的时间格点最大相差 ${clips[0].joints.max_timestamp_vs_video_grid_difference_s.toFixed(3)} / ${clips[1].joints.max_timestamp_vs_video_grid_difference_s.toFixed(3)} 秒。页面按导出帧索引关联视频，速度按原始时间戳计算；尚未独立核验硬件同步及相机/机器人外参。这里不计算或声称物理急动度改善。</p><p>优 / 差的方法身份按提供者标注。两段记录的平台任务编号为 ${clips[0].platform_task_id} / ${clips[1].platform_task_id}，相机视角不同；模型检查点、延迟和初始状态配对信息未提供。这是精选真机定性展示，视频时长不代表整体成功率。</p><p><a href="real_robot_manifest.json">原文件校验与数据清单</a> · <a href="real_robot/robot.urdf" download>URDF 模型</a> · <a href="real_robot/tcp_mapping.json">关节映射及 TCP 定义</a> · <a href="real_robot_tcp_verification.json">逐帧计算核验</a></p></details>`;
  ...
  function display() {
    ...
    section.querySelector('.step-readout').textContent = `${time.toFixed(2)} / ${endTime.toFixed(2)} 秒`;
    section.querySelectorAll('.real-clip-time').forEach((el, i) => {
      el.textContent = `${Math.min(time, lastTimes[i]).toFixed(2)} 秒${time >= lastTimes[i] - 1e-7 ? ' · 末帧保持' : ''}`;
    });
    tcpViewer?.draw(time);
  }
  function pause() {
